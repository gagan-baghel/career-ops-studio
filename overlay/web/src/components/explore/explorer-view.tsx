...
import { ATS_LABEL, type AtsSource } from "@/lib/explore";
...
export function ExplorerView({
  ...
  const { filters, setFilters, initFilters, phase, running, offers, discover, loadFresh, status, error, scannerMissing, mode, setMode, aiIntent, setAiIntent, discoverAI, companiesScanned, companiesAvailable, capHit, droppedNoDate, partial, partialSources } = useExplore();
  const partialNote = partialSources.length
    ? ` · ${partialSources.map((a: string) => ATS_LABEL[a as AtsSource] ?? a).join(", ")} stopped at the time limit (showing what it found)`
    : partial ? " · some boards were unreachable (normal for public directories)" : "";
  const scanNote =
    companiesScanned > 0
      ? `Scanned ${companiesScanned.toLocaleString()}${companiesAvailable > companiesScanned ? ` of ${companiesAvailable.toLocaleString()}` : ""} compan${companiesScanned === 1 ? "y" : "ies"}${partialNote}.`
      : undefined;
