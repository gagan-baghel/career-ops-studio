...
export type SourceState = {
  // "partial": the source hit its time budget — real results, but incomplete.
  state: "queued" | "active" | "swept" | "noisy" | "partial";
  companies?: number;
  done?: number;
  total?: number;
  matches?: number;
...
export function ExploreProvider({ children }: { children: React.ReactNode }) {
  ...
  const discover = useCallback(async () => {
    ...
    // Live matches reported by sources still scanning; a finished source's
    // offers are in `acc`. Counter = both, so it never drops as sources finish.
    const running: Record<string, number> = {};
    const bump = () => setMatchCount(acc.length + Object.values(running).reduce((a, b) => a + b, 0));
    let sawError = "";
        ...
        for (;;) {
          ...
          while ((nl = buf.indexOf("\n")) >= 0) {
            ...
            switch (ev.kind) {
              case "atsStart":
                setPhase("scanning");
                setStatus(`Walking ${f.ats.map((a) => ATS_LABEL[a] ?? a).join(", ")} in parallel…`);
                setSources((s) => ({ ...s, [ev.ats]: { ...s[ev.ats as AtsSource], state: "active", companies: ev.companies } }));
                break;
              case "progress":
                // Each source runs as its own scan; `matches` is that source's
                // running total until its offers arrive.
                running[ev.ats] = ev.matches;
                bump();
                setSources((s) => ({ ...s, [ev.ats]: { ...s[ev.ats as AtsSource], state: "active", done: ev.scanned, total: ev.total } }));
                break;
              case "atsDone":
                delete running[ev.ats];
                bump();
                setSources((s) => {
                  const next = { ...s, [ev.ats]: { ...s[ev.ats as AtsSource], state: ev.partial ? "partial" : ev.unreachable > 0 ? "noisy" : "swept", unreachable: ev.unreachable } } as typeof s;
                  // Sources scan in parallel now; name the ones still going.
                  const left = (Object.keys(next) as AtsSource[]).filter((k) => next[k]?.state === "active" || next[k]?.state === "queued");
                  if (left.length) setStatus(`Still walking ${left.map((k) => ATS_LABEL[k] ?? k).join(", ")}…`);
                  return next;
                });
                break;
              case "offer":
              ...
              case "summary": {
                ...
                if (ev.partialSources?.length) {
                  setPartialSources(ev.partialSources);
                  setPartial(true);
                }
  ...
  useEffect(() => {
    ...
    setPartialSources(Array.isArray(snap.partialSources) ? snap.partialSources : []);
  ...
  useEffect(() => {
    ...
    try {
      const snap: ResultSnapshot = {
        ...
        partial, partialSources, status, error, scannerMissing, added: [...added], aiTrace, aiCost, aiIntent,
      };
      sessionStorage.setItem(RESULTS_KEY, JSON.stringify(snap));
    } catch {
      /* sessionStorage full/unavailable — non-fatal */
    }
  }, [phase, mode, offers, matchCount, companiesScanned, companiesAvailable, capHit, droppedNoDate, sources, partial, partialSources, status, error, scannerMissing, added, aiTrace, aiCost, aiIntent]);

  const value = useMemo(
    () => ({
      filters, setFilters, initFilters, phase,
      running: phase === "casting" || phase === "scanning" || phase === "revealing" || phase === "hunting",
      offers, sources, matchCount, companiesScanned, companiesAvailable, capHit, droppedNoDate, status, partial, partialSources, error, scannerMissing, added, adding,
      discover, loadFresh, addToPipeline, applyPatch, reset,
      mode, setMode, aiIntent, setAiIntent, discoverAI, aiTrace, aiCost,
    }),
    [filters, setFilters, initFilters, phase, offers, sources, matchCount, companiesScanned, companiesAvailable, capHit, droppedNoDate, status, partial, partialSources, error, scannerMissing, added, adding, discover, loadFresh, addToPipeline, applyPatch, reset, mode, setMode, aiIntent, discoverAI, aiTrace, aiCost],
  );
