...
export function BetaBanner() {
  ...
  return (
    <>
      {/* Resting state is a bare circular bug icon so it covers no content.
          Hover or keyboard focus expands it to the version + label; the whole
          control is one button, so a click reports from either state. */}
      <button
        onClick={openReport}
        aria-label={`Report a bug — ${meta.version} ${meta.channel}`}
        title="Report a bug"
        className="group fixed bottom-3 left-3 z-[70] flex h-9 items-center overflow-hidden rounded-full border border-brand/30 bg-surface/90 px-[10px] text-xs shadow-lg backdrop-blur-md transition-[padding,border-color] duration-200 hover:border-brand/60 hover:pr-3 focus-visible:border-brand/60 focus-visible:pr-3 focus-visible:outline-none"
      >
        <Bug className="size-4 shrink-0 text-brand-text" />
        <span className="flex max-w-0 items-center gap-2 whitespace-nowrap pl-0 opacity-0 transition-[max-width,opacity,padding] duration-200 group-hover:max-w-[16rem] group-hover:pl-2 group-hover:opacity-100 group-focus-visible:max-w-[16rem] group-focus-visible:pl-2 group-focus-visible:opacity-100">
          <span className="font-medium text-brand-text">
            {meta.version} · {meta.channel}
          </span>
          {meta.sha && <span className="font-mono text-faint">{meta.sha}</span>}
