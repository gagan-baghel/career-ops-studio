  ...
  const startJob = useCallback(
    (opts: StartOpts): string | null => {
      ...
      (async () => {
        ...
        let closed = false; // the posting is dead — nothing was scored
        const steps: JobStep[] = [];
        ...
        try {
          ...
          if (closed) {
            // Drop the dead posting from the inbox so it isn't scored again.
            await fetch("/api/pipeline/remove", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ urls: [opts.input] }),
            }).catch(() => {});
            finish("done", "Posting is closed — removed from your inbox");
          } else finish("done", "Done");
        } catch {
