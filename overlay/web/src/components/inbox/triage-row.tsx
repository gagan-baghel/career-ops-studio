...
import { Bookmark, BookmarkCheck, Loader2, Send, X } from "lucide-react";
...
export function TriageRow({
  ...
  const hasUrl = !!job.url && /^https?:\/\//i.test(job.url);
    ...
    >
      ...
      {evaluated ? (
        ...
        <div className="flex shrink-0 items-center gap-1">
          {/* Apply straight from the inbox, without scoring first. There is no
              tailored CV for an unscored posting, so this leans on the résumé
              uploaded under CV → "Résumé to attach". Opens the form filled in;
              submitting is still yours. */}
          {hasUrl && (
            <button
              type="button"
              onClick={() => {
                const { pathname, search, hash } = window.location;
                void apply.open(job.url, {
                  prefill: true,
                  company: job.company,
              ...
              title="Apply — opens the form pre-filled with your answers and résumé. You review and submit."
              className="inline-flex items-center justify-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-muted transition-colors hover:bg-surface-hover hover:text-brand max-sm:min-h-[44px] max-sm:min-w-[44px]"
            >
