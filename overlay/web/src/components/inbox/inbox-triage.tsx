...
export function InboxTriage({ inbox }: { inbox: InboxJob[] }) {
  ...
  // Crossing a posting out removes it from data/pipeline.md. It used to only be
  // hidden in this browser, so the file kept every rejected posting and the
  // inbox count never moved. Undo restores the exact line the server removed.
  const removeFromPipeline = async (urls: string[], label: string) => {
    if (!urls.length) return;
    setHidden((h) => [...new Set([...h, ...urls])]); // optimistic: row leaves at once
    try {
      ...
      const lines: string[] = d.removed ?? [];
      setUndo({
        label,
        fn: () => {
          setHidden((h) => h.filter((u) => !urls.includes(u)));
          void fetch("/api/pipeline/remove", {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
      ...
      router.refresh(); // the header count is server-rendered from the file
    } catch {
      setHidden((h) => h.filter((u) => !urls.includes(u))); // put it back on failure
    }
  };

  const skip = (job: InboxJob) => {
    void removeFromPipeline([job.url], `Removed ${job.company}`);
  };

  const removeSelected = () => {
    const urls = [...selected];
    setSelected(new Set());
    void removeFromPipeline(urls, `Removed ${urls.length} posting${urls.length === 1 ? "" : "s"}`);
  };

  // Select-all acts on what is on screen, which is what the filters say — never
  // silently on the whole inbox behind them.
  const allVisibleSelected = visible.length > 0 && visible.every((e) => selected.has(e.job.url));
  const toggleSelectAll = () =>
    setSelected((s) => {
      const n = new Set(s);
      if (allVisibleSelected) for (const e of visible) n.delete(e.job.url);
      else for (const e of visible) n.add(e.job.url);
      return n;
    });
  ...
  return (
    <div className={cn("mx-auto mt-4 max-w-3xl", shortlist.length > 0 && "pb-28 sm:pb-24")}>
      ...
      <div className="mt-4 flex items-baseline justify-between gap-3">
        ...
        {hiddenCount > 0 && (
          // Postings hidden before removal became real are still in the file.
          // Rather than make the user cross out thousands again, offer both ways
          // out: delete them for good, or put them back.
          <span className="flex items-center gap-2 text-xs text-faint">
            {hiddenCount} hidden
            <button
              type="button"
              onClick={() => {
                const urls = hidden.slice();
                void removeFromPipeline(urls, `Removed ${urls.length} posting${urls.length === 1 ? "" : "s"}`);
              }}
              title="Delete every hidden posting from data/pipeline.md"
              className="underline underline-offset-2 transition-colors hover:text-amber-700 dark:hover:text-amber-400"
            >
              remove from pipeline
            </button>
            ·
            <button type="button" onClick={() => setHidden([])} className="underline underline-offset-2 transition-colors hover:text-foreground">
              restore
            </button>
          </span>
        )}
      ...
      {/* select-all + multi-select action bar */}
      {visible.length > 0 && (
        <div className="mt-2 flex flex-wrap items-center gap-3 rounded-lg border border-border px-3 py-2 text-sm">
          ...
          {selected.size > 0 && (
            <>
              ...
              <button
                ...
                title="Delete these from data/pipeline.md"
                className="rounded-md border border-border px-2.5 py-1 text-xs font-medium text-muted transition-colors hover:border-amber-500/50 hover:text-amber-700 max-sm:min-h-[44px] dark:hover:text-amber-400"
              >
