...
type ProfilePatch = {
  ...
  currency?: string;
  remote?: string;
  // Added for the profile form. compRange is free text on purpose: real ranges
  // carry a unit ("18-30 LPA", "$120K-$160K"), and rebuilding one from two
  // numbers drops it. compMin/compMax stay for the assistant's numeric path.
  phone?: string;
  linkedin?: string;
  github?: string;
  portfolio?: string;
...
function patchToProfile(p: ProfilePatch): Record<string, unknown> {
  ...
  if (p.phone !== undefined) candidate.phone = p.phone;
  if (p.linkedin !== undefined) candidate.linkedin = p.linkedin;
  if (p.github !== undefined) candidate.github = p.github;
  if (p.portfolio !== undefined) candidate.portfolio_url = p.portfolio;
  ...
  // An explicit range string wins over the numeric pair — it keeps the unit.
  if (p.compRange !== undefined) comp.target_range = p.compRange;
  else if (p.compMin && p.compMax) comp.target_range = `${p.compMin}-${p.compMax}`;
  if (p.compMinimum !== undefined) comp.minimum = p.compMinimum;
...
// Read back exactly the fields the form owns, so it edits what is on disk
// instead of starting blank and deep-merging placeholders over real data.
export async function GET() {
  const file = path.join(careerOpsRoot(), "config", "profile.yml");
  if (!fs.existsSync(file)) return Response.json({ exists: false });
  let doc: unknown;
  try {
    doc = yaml.load(fs.readFileSync(file, "utf8"));
  } catch {
    return Response.json(
      { error: "config/profile.yml is not valid YAML — fix it before editing here." },
      { status: 409 },
    );
  }
  const d = isObj(doc) ? doc : {};
  const candidate = isObj(d.candidate) ? d.candidate : {};
  const comp = isObj(d.compensation) ? d.compensation : {};
  ...
  return Response.json({
    ...
    portfolio: str(candidate.portfolio_url),
    compRange: str(comp.target_range),
    currency: str(comp.currency),
    compMinimum: str(comp.minimum),
    remote: str(comp.location_flexibility),
  });
}
