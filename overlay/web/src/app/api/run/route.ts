...
export async function POST(req: Request) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      let textTail = ""; // last bit of agent text, to spot a "VERDICT: CLOSED" dead posting
      ...
      const sendAgentText = (text: string) => {
        textTail = (textTail + text).slice(-2000);
      ...
      child.on("close", (code) => {
        ...
        } else if (persists && !wroteReport && /VERDICT:\s*(CLOSED|N\/A)/i.test(textTail)) {
          // Dead posting: the liveness gate says write nothing — a correct outcome, not a failure.
          send({ type: "done", closed: true, tokens: lastTokens, costUsd: lastCostUsd });
