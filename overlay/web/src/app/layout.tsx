...
// Also default the AI to local Claude Code (Config can change it) — before any
// component reads career-ops:config on mount.
const THEME_SCRIPT = `(function(){try{var t=localStorage.getItem('career-ops:theme');var d=t==='dark'||(!t&&window.matchMedia('(prefers-color-scheme: dark)').matches);if(d)document.documentElement.classList.add('dark');var m=document.querySelector('meta[name="theme-color"]');if(!m){m=document.createElement('meta');m.setAttribute('name','theme-color');document.head.appendChild(m);}m.setAttribute('content',d?'#0a0a0a':'#f7f6f3');var k='career-ops:config',c=JSON.parse(localStorage.getItem(k)||'{}');if(!c.cliId){c.mode=c.mode||'cli';c.cliId='claude';localStorage.setItem(k,JSON.stringify(c));}}catch(e){document.documentElement.classList.add('dark');}})();`;
