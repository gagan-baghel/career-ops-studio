...
/**
 * The candidate's own uploaded résumé — what gets attached when career-ops has
 * not generated a tailored CV for this company. Deliberately NOT part of
 * resolveTailoredCv: "download the tailored CV" must never quietly hand back a
 * generic file, so only the apply flow falls back to this.
 */
export function resolveDefaultResume(): string | null {
  const dir = path.join(careerOpsRoot(), "documents");
  const allowed = [".pdf", ".docx", ".doc", ".rtf", ".txt"];
  try {
    const hit = fs
      .readdirSync(dir)
      .find((f) => path.parse(f).name === "application-resume" && allowed.includes(path.extname(f).toLowerCase()));
