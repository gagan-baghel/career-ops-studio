...
export function readInbox(): InboxJob[] {
  ...
  for (const line of md.split("\n")) {
    ...
    if (!m) continue;
    // `~~URL | …~~` is how the pipeline records a closed posting. Read it as
    // done with the wrapper stripped — not as a live row with a "~~https" URL.
    const struck = m[2].trimStart().startsWith("~~");
    const body = struck ? m[2].trim().slice(2).replace(/~~[\s\S]*$/, "") : m[2];
    const all = body.split("|").map((s) => s.trim());
