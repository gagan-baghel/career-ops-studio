...
// Optional wall-clock budget for the whole sweep (CAREER_OPS_SCAN_BUDGET_MS).
// An interactive caller (the web UI) cannot wait out a 12k-company sweep: one
// large Workday tenant alone can hold COMPANY_TIMEOUT_MS, and a killed --json
// run loses every match it found. With a budget the sweep stops starting new
// companies once it is spent, clamps in-flight ones to what is left, and still
// writes its result — reporting budgetHit so the caller knows it is partial.
// Unset (the CLI default) keeps the unbounded behaviour.
export function scanBudget(env = process.env, now = Date.now()) {
  const ms = Number(env.CAREER_OPS_SCAN_BUDGET_MS);
  if (!Number.isFinite(ms) || ms <= 0) return { spent: () => false, companyTimeoutMs: () => COMPANY_TIMEOUT_MS };
  const deadline = now + ms;
  return {
    spent: () => Date.now() >= deadline,
    // +50ms: a timer can fire a hair before Date.now() reaches the deadline,
    // and a company cut by the budget must read as spent(), not as unreachable.
    companyTimeoutMs: () => Math.max(1_000, Math.min(COMPANY_TIMEOUT_MS, deadline - Date.now() + 50)),
  };
}
...
async function main() {
  ...
  for (const name of opts.ats) {
    ...
    let errors = 0;
    // Companies the time budget cut off mid-fetch: not unreachable, not dead.
    let budgetCut = 0;
    ...
    await parallelEach(entries, source.concurrency ?? CONCURRENCY, async (entry) => {
      ...
      try {
        ...
        })(), budget.companyTimeoutMs(), `${name}/${entry.name}`);
      } catch (err) {
        if (budget.spent()) {
          budgetCut++;
          return;
      ...
      // Every ~5% (at most every 200): a 150-company Workday pass used to
      // report nothing between 0 and done, which reads as a hung scan.
      if (done % Math.max(1, Math.min(200, Math.ceil(entries.length / 20))) === 0 || done === entries.length) {
    ...
    if (truncated.length && !resolverOutage && !budget.spent()) {
      ...
      for (const entry of truncated) {
        ...
        try {
          ...
          })(), budget.companyTimeoutMs(), `${name}/${entry.name} (retry)`);
        } catch (err) {
          errors++;
    ...
    if (budgetCut || (budget.spent() && lastDone < entries.length)) {
      // Same accounting as the outage stop above: companies never contacted,
      // or cut off mid-fetch, are not "scanned", and a non-dry run keeps a
      // resume point. ponytail: the resume point is the lowest unfinished
      // index, so boards cut mid-fetch below it are not retried by --resume;
      // the budget is meant for interactive --dry-run callers.
      budgetHit = true;
      totalCompaniesScanned -= entries.length - lastDone + budgetCut;
      if (!opts.dryRun) {
        writeCheckpoint({
          ...checkpointBase(),
          current: { name, resumeAt: startAt + lastResumeAt, datasetLen: list.length, datasetHash },
      ...
      log(`\n  ⏱  ${name}: time budget spent — ${lastDone - budgetCut} of ${entries.length} companies fully scanned, partial result.`);
      break;
    }
  ...
  if (!opts.dryRun && !stoppedByOutage && !budgetHit && existsSync(CHECKPOINT_PATH)) unlinkSync(CHECKPOINT_PATH);
  ...
  if (opts.json) {
    process.stdout.write(JSON.stringify({
      ...
      stoppedByOutage,
      // Stopped by CAREER_OPS_SCAN_BUDGET_MS before every company was reached.
      budgetHit,
      datasetStatus,
