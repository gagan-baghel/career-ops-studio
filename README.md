...
| `lib/core/scan.ts` + `explore/*` | **Explore finishes, and keeps what it found.** The scan ran every source in one process and was killed at 280 s. Since a `--json` run only writes results at the end, one slow Workday tenant threw away the Greenhouse, Lever and Ashby matches too. Now each source scans in its own process, in parallel, under a 3-minute budget. Each source's results show as soon as it finishes, the chips show live counts, and a source cut short says "time limit" rather than failing. Scans also pass `--shuffle`: `--limit` without it took the first N companies alphabetically, so every scan searched the same "A…" employers. |
| `scan-ats-full.mjs` (engine) | Adds `CAREER_OPS_SCAN_BUDGET_MS`, off by default so the CLI is unchanged. Once the budget is spent the sweep stops starting companies, cuts in-flight ones off at the deadline, skips the slow sequential retry, and still writes its result, flagged `budgetHit`. Companies cut off are counted as budget cuts, not as unreachable or dead boards. Progress prints every ~5% instead of every 200 companies. Built on the existing `shouldStop` hook the DNS-outage breaker uses. Test: `tests/scan-ats-full-budget.test.mjs`. |
| `lib/core/portals-serialize.mjs` | With no keywords set, Explore wrote a comment-only filter file. `js-yaml` 5 rejects that as "no document", so the scan died with "Fatal". |
